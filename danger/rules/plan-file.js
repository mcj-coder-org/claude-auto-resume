...
/**
 * Search all plan files for one matching the linked issue
 * @param {string} issueNumber - Issue number to find
 * @returns {Promise<{path: string, content: string}|null>}
 */
async function findPlanByIssue(issueNumber) {
  // This would require filesystem access or GitHub API
  // For now, we rely on modified files detection
  // Full implementation would scan docs/plans/**/*.md
  return null;
}
 ...
 */
export async function validatePlanFiles() {
  ...
  if (modifiedPlanFiles.length === 0 && linkedIssue) {
    const planByIssue = await findPlanByIssue(linkedIssue);
    if (!planByIssue) {
      warn(
